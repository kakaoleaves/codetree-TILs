...
{
	int rx = red_pos.first;
	int ry = red_pos.second;
	int bx = blue_pos.first;
	int by = blue_pos.second;
...
void Push(pair<int, int> r_pos, pair<int, int> b_pos, int new_step)
{
	q.push(make_pair(red_pos, blue_pos));
	visited[r_pos.first][r_pos.second][b_pos.first][b_pos.second] = true;
	step[r_pos.first][r_pos.second][b_pos.first][b_pos.second] = new_step;
}
